...
class MindMapInteractiveLearningTests(TestCase):
    ...
    @patch("apps.courses.utils.generate_text_with_fallback")
    def test_get_node_content_on_demand_generation(self, mock_ai):
        ...
        result = self.service.get_node_content(
            str(self.mind_map.uuid), "lesson_1", self.user
        )
    ...
    def test_get_or_create_node_quiz_on_demand_generation(self, mock_ai):
        ...
        # Fetch the quiz
        result = self.service.get_or_create_node_quiz(
            str(self.mind_map.uuid), "lesson_1", self.user
        )
        ...
        self.assertEqual(
            updated_map.quizzes["lesson_1"]["questions"][0]["correct_answer"],
            "Verdadeiro",
        )
    ...
    def test_submit_quiz_passing_and_progression_unlock(self):
        ...
        res = self.service.submit_node_quiz(
            str(self.mind_map.uuid), "lesson_1", submission, self.user
        )
    ...
    def test_update_node_note_autosave(self):
        ...
        result = self.service.update_node_note(
            str(self.mind_map.uuid), "lesson_1", note_content, self.user
        )
    ...
    def test_robust_extract_json(self):
        ...
        from apps.courses.utils import extract_json
        ...
        res = extract_json(
            '{"text_content": "Auto repaired content", "additional_resources": []'
        )
        ...
        res = extract_json(
            '{"text_content": "Truncated content without quote and brace'
        )
        self.assertEqual(
            res["text_content"], "Truncated content without quote and brace"
        )
    ...
    def test_sharing_access_control(self):
        ...
        with self.assertRaises(HttpError):
            self.service.update_node_note(
                str(self.mind_map.uuid), "lesson_1", "Test note", other_user
            )

        with self.assertRaises(HttpError):
            self.service.submit_node_quiz(
                str(self.mind_map.uuid), "lesson_1", {"1": "True"}, other_user
            )

    def test_duplicate_mind_map(self):
        """Verify that duplicating a shared mind map clones it for the new user with empty progress."""
        other_user = User.objects.create_user(
            username="otheruser2", email="other2@example.com", password="Password123"
        )
