...
from apps.mind_maps.services import MindMapService
...
class FolhaService:
    ...
    async def _get_owned_folha(self, folha_uuid: str, user):
        ...
        try:
            folha = await sync_to_async(
                Folha.objects.select_related("mind_map").get
    ...
    async def _resolve_mind_map(self, mind_map_id: str, user):
        try:
            return await sync_to_async(MindMap.objects.get)(
                uuid=mind_map_id, user=user
            )
    ...
    async def list_folhas(
        ...
        queryset = Folha.objects.filter(user=user).select_related("mind_map").only(
            "uuid", "title", "content", "mind_map", "node_id", "created_at", "updated_at"
        )
        if mind_map_id:
            queryset = queryset.filter(mind_map__uuid=mind_map_id)
        if node_id:
            queryset = queryset.filter(node_id=node_id)
        folhas = await sync_to_async(lambda: list(queryset[:100]))()
        cache.set(cache_key, folhas, 300)
        return folhas

    async def get_folha(self, folha_uuid: str, user):
        return await self._get_owned_folha(folha_uuid, user)

    async def get_folha_for_node(self, user, mind_map_id: str, node_id: str):
        ...
        try:
            folha = await sync_to_async(
                Folha.objects.select_related("mind_map").get
            )(user=user, mind_map__uuid=mind_map_id, node_id=node_id)
            cache.set(cache_key, folha, 600)
    ...
    async def create_folha(
        ...
        if mind_map_id or node_id:
            ...
            mind_map = await self._resolve_mind_map(mind_map_id, user)
            self._validate_node_link(mind_map, node_id)

            existing = await sync_to_async(
                Folha.objects.filter(user=user, mind_map=mind_map, node_id=node_id).first
            )()
            if existing:
                return existing

        try:
            folha = await sync_to_async(Folha.objects.create)(
                user=user,
            ...
            if mind_map and node_id:
                folha = await sync_to_async(Folha.objects.get)(
    ...
    async def update_folha(
        self, folha_uuid: str, user, title: str | None = None, content: str | None = None,
    ):
        folha = await self._get_owned_folha(folha_uuid, user)
        if title is not None:
            folha.title = title
        if content is not None:
            folha.content = content
        await sync_to_async(folha.save)()
    ...
    async def delete_folha(self, folha_uuid: str, user):
        folha = await self._get_owned_folha(folha_uuid, user)
        mind_map_id = str(folha.mind_map.uuid) if folha.mind_map_id else None
        node_id = folha.node_id
        await sync_to_async(folha.delete)()
        invalidate_folha_cache(
