...
def send_receipt_notification_task(user_id, plan_id, receipt_url, secret_token=""):
    try:
        ...
        approve_url = (
            f"{api_url}/subscriptions/manual/receipts/{secret_token}/approve"
            if secret_token
            else ""
        )
        reject_url = (
            f"{api_url}/subscriptions/manual/receipts/{secret_token}/reject"
            if secret_token
            else ""
        )
        send_receipt_notification_email(
